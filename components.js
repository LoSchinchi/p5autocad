//--- classe Punto: istanzia un punto
//- (number, number) x, y sono la x e la y geometrica
//- (number, number) xzoom, yzoom servono per cambiare x e y in base alla scala
//- (boolean) hidden dice se un punto si stampa
//- constructor(x: number, y: number, hidden: boolean=true)
//- (void) draw(), disegna il punto
//- (void) drawSelected(), disegna il punto se selezionato
//- (void) checkEvts(), controlla se il punto è selezionato e decide se eseguire this.draw() o this.drawSelected()
//- (boolean) collide(), ritorna true se il mouse passa sopra il punto
//- (number, number) getX(), getY() restituiscono le coordinate del canvas in base al valore di this.x, this.y e della posizione degli assi
class Punto {
    ...
    collide = () => dist(mouseX, mouseY, this.getX(), this.getY()) <= max(10, 10 * scala) / 2;

    getX = () => this.x + greenLineX * scala;

    getY = () => this.y + redLineY * scala;
}

//--- classi Line, Ellisse, Rombo, Circonf, Arc, Rectangle, Polig, Spline
//- ([Punto, ...] | { centro: undefined | Punto, punti | angles: [Punto, ...] } | { angles: [Punto, ...], mids: : [Punto, ...] }) oggetto contenente tutti i punti dell'elemento
//- ({ dashed: boolean, mode: string, weight: null | number, colorStroke: null | string, ?fillColor: null | string, ?width: null | number, ?height: null | number, ?diametro: null | number, ?raggio: null | number, ?nLati: null | number, ?angle: null | number }) oggetto contenente le caratteristiche dell'elemento
//- (void) draw(), disegna l'elemento in base a this.punti e this.caracts
//- (boolean) collide(), ritorna true se il mouse passa sopra l'elemento
//- (void) apply(), applica le modifiche all'elemento

//- (boolean) __collideLine(p1: Punto, p2: Punto), vede se il mouse è sopra la linea che passa tra p1 e p2
    ...
    draw() {
        if(elementSelected === this) {
            ...
            for(let i = 0; i < this.punti.length; i ++) {
                ...
                this.punti[i].xzoom = puntiElSelected[i].xzoom + m.x;
                this.punti[i].yzoom = puntiElSelected[i].yzoom + m.y;
            }
...
class Ellisse {
    ...
    constructor(c, p1) {
        ...
        this.punti.punti.forEach(e => punti.push(e));
    }
    ...
    draw() {
        ...
        this.punti.centro.hidden = false; // sicuro che sia visibile 
...
class Rombo {
    ...
    collide = () => collidePolygon(this.punti.punti);
...
//- constructor(p: Punto, raggio: number, actCarica: null | { dashed: boolean, mode: string, weight: number, colorStroke: string, fillColor: null | string, diametro: number }
//- (void) __setAngle(onCarica: boolean), instanzia i punti corrispondenti agli angoli noti ogni PI/4, se onCarica == false elimina i punti prima di crearli
class Circonf {
    punti = {
        centro: undefined,
        angles: []
...
//- constructor(centro: Punto, p1: Punto, p2: Punto, raggio: number)
class Arc {
    punti = {
        centro: undefined,
        punti: []
...
//- (void) __setMids(p1: Punto, p2: Punto), istanzia il punto in mezzo tra due vertici
//- (void) __setMids3p(p1: Punto, p2: Punto, p3: Punto, p4: Punto), istanzia i punti in mezzo per ogni lato formato da 2 dei 4 punti
//- constructor(p1: Punto, p2: Punto, p3: null | Punto | number, p4: null | Punto | number)
class Rectangle {
    punti = {
        angles: [],
        mids: []
    ...
    constructor(p1, p2, p3=null, p4=null) {
        ...
        this.punti.mids.forEach(e => punti.push(e));
    }
...
//- constructor(centro: Punto, punto1: Punto, caract: null | { dashed: boolean, mode: string, weight: null | number, colorStroke: null | string, fillColor: string, diametro: null | number, nLati: number, angle: number })
//- (void) __setPuntiPolInsc(p: Punto), instanzia i punti del poligono inscritto a partire da p in base a this.caract.nLati e this.caract.angle
//- (void) __setPuntiPolCirc(p: Punto), instanzia i punti del poligono circoscritto a partire da p in base a this.caract.nLati e this.caract.angle
//- (number) __getAlfa(o: Punto), restituisce il grado in radianti tra il punto p e il centro
class Polig {
    punti = {
        centro: null,
        angles: []
    ...
    draw() {
        if(elementSelected === this) {
            ...
            this.caract.mode = 'create pol inscr';
            let _raggio = document.getElementsByName('raggio')[0].value;
            ...
            if(__n !== this.caract.nLati)
                setPuntiElSelected(this.punti, this.caract.mode); // perché se si aggiunge un lato aumentano i punti, nell'if perchè sennò i vecchi punti cambiano sempre
            setMovesConCentro(this.punti);
            this.punti.angles.forEach(e => punti.push(e));
            punti.push(this.punti.centro);
            setCaractDaInput();
    ...
    collide = () => collidePolygon(this.punti.angles);
...
class Spline {
    ...
    constructor(arrP) {
        ...
        this.punti.forEach(e => punti.push(e));
...
//--- (boolean) collidePolygon(arrP) ritorna true se il mouse passa dentro un poligono
//- ([Punto, ...]) arrP, array dei punti esterni del poligono
...
//--- (void) setCaractsLine(car) setta i caratteri della linea o della spline su car
//- ({ dashed: boolean, mode: string, weight: number, colorStroke: string }) car, è l'oggetto on i caratteri
...
//--- (void) setCaracts(car) setta i caratteri di un elemento
//- ({ dashed: boolean, mode: string, weight: number, colorStroke: string , fillColor: null | string }) car, è l'oggetto on i caratteri
...
//--- (void) setCaractDaInput(), imposta sul canvas i caratteri presi dagli input
...
//--- (void) deletePunti(p, mode) elimina i punti di un elemento
//- ([Punto, ...] | { centro: undefined | Punto, punti: [Punto, ...] } | { centro: undefined | Punto, angles: [Punto, ...] } |  { angles: [Punto, ...], mids: [Punto, ...]}) p, è l'oggetto contenente i punti
//- (string) mode, è l'azione con cui è stato creato l'elemento
function deletePunti(p, mode) {
    let arrPunti;
    if(mode === 'create line')
        arrPunti = p;
...
//--- (void) deleteElementSelected() elimina l'elemento selezionato (se si preme Canc o il <button> delete)
function deleteElementSelected() {
    ...
    /*else if(caractsElSeletced.mode.indexOf(' spline ') !== -1)
        ...
        arrT = objs;
    deletePunti(elementSelected.punti, elementSelected.caract.mode);
...
//--- (void) applyModify() applica le modifiche all'elemento selezionato
...
//--- ({ x: number, y: number}) getMoves() restituisce di quanto si deve spostare l'elemento
function getMoves() {
    let _moveX = document.getElementsByName('xMoved')[0].value;
    let moveX = (_moveX === '')? 0: parseFloat(_moveX) * SCALA_1_CM * scala;
    let _moveY = document.getElementsByName('yMoved')[0].value;
    let moveY = (_moveY === '')? 0: -parseFloat(_moveY) * SCALA_1_CM * scala;
...
//--- (void) setMovesConCentro(p), cambia le coordinate dei punti per circonferenze, rettangoli e poligoni
//- ({ centro: undefined | Punto, angles: [Punto, ...] }) p, è l'oggetto contenente i punti
...
//--- (void) setMovesConCentroEllRombo(p), cambia le coordinate dei punti per ellissi e rombi
//- ({ centro: undefined | Punto, punti: [Punto, ...] }) p, è l'oggetto contenente i punti
...
//--- (void) setPuntiElSelected(pti, mode), setta la variabile puntiElSelected in base a mode e a pti
//- ([Punto, ...] | { centro: undefined | Punto, punti | angles: [Punto, ...] } | { angles: [Punto, ...], mids: : [Punto, ...] }) pti, sono i punti dell'elemento selezionato
//- (string) mode, il tipo di elemento selezionato
function setPuntiElSelected(pti, mode) {
    if(mode === 'create line') {
        puntiElSelected = [];
        pti.forEach(p => {
            let t = new Punto(p.x, p.y, p.hidden);
            puntiElSelected.push(t);
            punti.push(t);
        });
    } else if(mode === 'create rombo' || mode === 'create ellipse' || mode.indexOf(' arc ') !== -1) {
        puntiElSelected = {
            centro: new Punto(pti.centro.x, pti.centro.y, pti.centro.hidden),
            punti: []
        };
        pti.punti.forEach(p => puntiElSelected.punti.push(new Punto(p.x, p.y, p.hidden)));
    } else if(mode.indexOf(' ci ') !== -1 || mode.indexOf(' pol ') !== -1) {
        puntiElSelected = {
            centro: new Punto(pti.centro.x, pti.centro.y, pti.centro.hidden),
            angles: []
        };
        pti.angles.forEach(p => puntiElSelected.angles.push(new Punto(p.x, p.y, p.hidden)));
    } else {
        puntiElSelected = {
            angles: [],
            mids: []
        ...
        pti.angles.forEach(p => puntiElSelected.angles.push(new Punto(p.x, p.y, p.hidden)));
        pti.mids.forEach(p => puntiElSelected.mids.push(new Punto(p.x, p.y, p.hidden)));
    }
}