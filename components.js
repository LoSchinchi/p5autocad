...
class Ellisse {
    ...
    collide() {
        let rx = this.caract.width * scala / 2 * SCALA_1_CM, ry = this.caract.height * scala / 2 * SCALA_1_CM;
        let cx = this.punti.centro.getX(), cy = this.punti.centro.getY();
        let x = mouseX, y = mouseY;

        if (x > cx + rx || x < cx - rx || y > cy + ry || y < cy - ry || (y >= cy - 3 && y <= cy + 3 && x >= cx - 3 && x <= cx + 3))
            return false;

        let xx = x - cx, yy = y - cy;
        let eyy = ry * sqrt(abs(rx * rx - xx * xx)) / rx;
...
class Circonf {
    ...
    constructor(p, raggio) {
        if(action === 'create ci 2p' || action === 'create ci 3p')
            punti.push(p);

        setCaracts(this.caract);
        this.caract.mode = String(action);
        this.caract.diametro = raggio * 2 / SCALA_1_CM;
        this.punti.centro = p;
        this.__setAngle();
    }
...
function collidePolygon(points) {
    let collision = false, next = 0, y = mouseY;
    ...
    for (let current = 0; current < points.length; current ++) {
        ...
        let vc = points[current];    // c for "current"
        let vn = points[next];       // n for "next"

        // compare position, flip 'collision' variable back and forth
        if (((vc.getY() >= y && vn.getY() < y) || (vc.getY() < y && vn.getY() >= y)) && (mouseX < (vn.getX() - vc.getX()) * (y - vc.getY()) / (vn.getY() - vc.getY()) + vc.getX()))
            collision = !collision;
    }
    return collision;
}
