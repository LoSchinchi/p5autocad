...
let precX, precY;       // (number, number) x e y = a mouseX, mouseY al frame immediatamente precedente per quando si muove il canvas
let xStart, yStart;     // (number, number) x e y = a mouseX, mouseY al frame immediatamente precedente per quando si muove un elemento

//--- (void) evento per la rotella del mouse
//- (Event) event, è l'evento della rotella
function mouseWheel(event) {
    if(mouseOutofCanvas() || elementSelected !== undefined)
        return;
...
function setMousePressed() {
    ...
    if(mouseIsPressed && mouseButton === 'left' && elementSelected !== undefined && (elementSelected.collide() || nTimePressed >= 5) && !inCreation && !canvaInMovimento) {
        if(nTimePressed < 5) {
            ...
            cursor('grab');
            let addX = int(mouseX - xStart);
            let addY = int(yStart - mouseY);
            document.getElementsByName('xMoved')[0].value = (addX / scala / SCALA_1_CM).toFixed(3).toString();
            document.getElementsByName('yMoved')[0].value = (addY / scala / SCALA_1_CM).toFixed(3).toString();
        }
        ...
        if(possibleElementSelected !== undefined && elementSelected === undefined) {
            ...
            setPuntiElSelected(elementSelected.punti, elementSelected.caract.mode);
...
function keyPressed(event) {
    ...
    if(event.key === 'Escape') {
        ...
        if(elementSelected !== undefined) {
            ...
            elementSelected.punti = puntiElSelected;
        }
        elementSelected = undefined;
